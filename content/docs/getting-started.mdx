...
<Callout title="¿Dónde poner este código?">
  Recomendamos crear una carpeta `lib` o `services` en la raíz de tu proyecto
...
export const myDbAdapter: IDatabaseAdapter = {
  ...
  async saveUserProfile(
    ...
    console.log(
      `[DB Adapter] Guardando perfil para el usuario: ${profile.userId}`
    );
...
loyaltySystem.on("tier_changed", ({ userId, to }) => {
  if (to) {
    console.log(
      `¡EVENTO! El usuario ${userId} ha ascendido al nivel ${to.name}. ¡Enviando email de felicitación!`
    );
...
loyaltySystem.on("points_updated", ({ userId, newBalance, action }) => {
  console.log(
    `¡EVENTO! Puntos actualizados para ${userId} por "${action}". Nuevo saldo: ${newBalance}`
  );
});
```
